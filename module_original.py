# -*- coding: utf-8 -*-
"""
Created on Wed Jun 30 16:51:46 2021

@author: user

 worker function of camera module '筆電上原始鏡頭' 
"""

import cv2
import numpy as np
import threading
import queue # 儲存 image 

CHILD_DEAD = False # 確認 child thread 是否結束並等待 join
CAM_SAFETY = False # 確認鏡頭是否安全開啟
EXIT = False # 行程是否關閉
queue = queue.Queue( maxsize = 2 ) # 存放 RGB 圖片的佇列
...
def open_camera():
    global CAM_SAFETY
    cam = cv2.VideoCapture( 0, cv2.CAP_DSHOW ) # 取得筆電原始鏡頭存取資料
    
    if cam.isOpened() :
        print('module_original.py: camera open successfully')
        CAM_SAFETY = True
        return cam
    else:
        print('module_original.py: camera open failure')
        CAM_SAFETY = False
        return None
...
def grab_image( cam ):
    ...
    while( EXIT == False ):
        ...
        set_image( image ) # 儲存圖像
        
    
    print('module_original, grab_image(): pass while loop')
...
def worker_function():
    ...
    if( cam is None ):
        print('module_original, worker_function(): camera open failure, return now')
        CAM_SAFETY = False
        CHILD_DEAD = True
        return 
...
def worker_dead():
    ...
    if CHILD_DEAD == True:
        print('module_original: already dead')
        return True
    else:
        return False
...
def demo_display():
    ...
    while( True ):
        image = queue.get()
        cv2.imshow( 'module_original demo display', image )
